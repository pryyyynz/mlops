...
def train_and_log_model(data_path, params):
    ...
    with mlflow.start_run():
        ...
        # Log validation and test RMSE
        val_rmse = mean_squared_error(y_val, rf.predict(X_val), squared=False)
        mlflow.log_metric("val_rmse", val_rmse)
        test_rmse = mean_squared_error(y_test, rf.predict(X_test), squared=False)
        mlflow.log_metric("test_rmse", test_rmse)

        # Return the test RMSE and the current run ID
...
def run_register_model(data_path: str, top_n: int):
    ...
    # Evaluate these top models on the test set and log them to the new experiment
    best_run_id = None
    ...
    if best_run_id:
        ...
        print(f"Model registered with URI: {model_uri} and name: {MODEL_NAME}")
